    ...
    <div class="container px-6 mx-auto grid">
        ...
        <div class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            <a class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
                href="{{ route('admin.users.create') }}">Thêm Tài Khoản</a>
        </div>
                    ...
                    <table class="w-full whitespace-no-wrap">
                        <thead>
                            <tr
                                ...
                                <td class="px-4 py-4">Invoice No.</td>
                                <td class="px-4 py-4">Gender</td>
                                <td class="px-4 py-4">Role</td>
                                <td class="px-4 py-4 " colspan="3">Action</td>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @foreach ($data as $user)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-4">{{ $user->invoices->count() }}</td>
                                    <td class="px-4 py-4">
                                        {{ $user->gender == config('common.user.gender.male') ? 'Nam' : 'Nữ' }}</td>
                                    <td class="px-4 py-4">
                                        {{ $user->role == config('common.use.role.user') ? 'User' : 'Admin' }}</td>
                                    <td><a class="flex items-center justify-between px-2 py-2  text-purple-600  "
                                            aria-label="Edit"
                                            href="{{ route('admin.users.edit', ['user' => $user->id]) }}"><svg
                                    ...
                                    <td x-data="{show:false}" x-bind:class="{ 'model-open': show }"
                                        ...
                                        <div>
                                            <button @click={show=true} type="button"
                                                ...
                                                data-target="#confirm_delete_{{ $user->id }}"><svg class="w-6 h-6"
